...
class NISPANet(nn.Module):
    ...
    def __init__(self, classes_per_task=5, num_classes=100, scenario='class_il',
        ...
        # two 2x2 max-pools, so the spatial side is input_size // 4.
        side = input_size // 4
        self.fc1 = MaskedLinear(128 * side * side, 1024)
...
def build_nispa_model(dataset, classes_per_task, scenario, num_classes=None):
    """The official CIFAR NISPA ConvNet, resized for the other benchmarks.

    Only the input side (which sets the fc1 fan-in) and the output width change;
    the channel widths are the published ones.
    """
    from models import _INPUT_SIZE
    from datasets import ContinualLearningBenchmark
    dataset = dataset.lower()
    if dataset not in _INPUT_SIZE:
        raise ValueError(f'no NISPA backbone geometry for {dataset!r}')
    total = num_classes or ContinualLearningBenchmark.NUM_CLASSES[dataset]
    return NISPANet(classes_per_task, total, scenario, input_size=_INPUT_SIZE[dataset])
