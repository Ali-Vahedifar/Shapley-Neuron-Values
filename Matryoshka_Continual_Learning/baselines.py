...
from baselines_sparse import NISPA, PEC, WSN, SpaceNet, build_nispa_model
from method_loader import load as _load_method

_mcl_mod = _load_method('MCL', 'mcl')
...
def build_custom_model(method: str, dataset: str, classes_per_task: int,
    ...
    if method == 'pec':
        from PEC.pec import PECNet
        return PECNet(classes_per_task * num_tasks, classes_per_task)
    if method == 'nispa':
