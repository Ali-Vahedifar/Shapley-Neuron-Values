...
# GTEP splits a dataset into two disjoint halves, D_HT (tuning) and D_E
# (reporting).  Every 50/5 below used to be CIFAR-100's half geometry; it is
# derived instead, so a dataset swap is one flag.
DATASET_CLASSES={'cifar20':20,'cifar100':100,'tinyimagenet':200,'imagenet1k':1000}
# Task count per dataset: 10 everywhere except CIFAR-20, whose half is only ten
# classes (10 tasks would be one class per task).  GTEP_TASKS overrides.
DEFAULT_TASKS={'cifar20':5,'cifar100':10,'tinyimagenet':10,'imagenet1k':10}
TASKS=int(os.environ.get('GTEP_TASKS','10'))  # protocol constant; --tasks only truncates for pilots


def task_count(dataset='cifar100'):
    override=os.environ.get('GTEP_TASKS')
    return int(override) if override else DEFAULT_TASKS.get(dataset,TASKS)


def geometry(dataset,tasks=None):
    """(classes per half, classes per task) for one GTEP half."""
    if dataset not in DATASET_CLASSES:
        raise ValueError(f'unknown dataset {dataset!r}; expected one of {sorted(DATASET_CLASSES)}')
    tasks=tasks or task_count(dataset)
    half=DATASET_CLASSES[dataset]//2
    if half%tasks:
        raise ValueError(f'{dataset}: {half} classes per half do not divide into {tasks} tasks '
                         f'(set GTEP_TASKS)')
    return half,half//tasks
LR=('logu',1e-4,1e-2)
SPACE={
...
# Every file that can change a result.  SNV-A's two files and the runner that
# builds it are in the list: a fingerprint that omits them cannot tell an SNV-A
# run from a plain-SNV one.
SOURCE_FILES=('cl_base.py','models.py','datasets.py','metrics.py',
    'training_policy.py','audit_cost.py','train.py','audited_gtep.py',
    'SNV/snv_core.py','SNV/snv_adaptive.py','snv_adaptive_run.py','snv_core.py',
    'baselines/__init__.py','baselines/regularization.py','baselines/sparse.py',
...
def make_method(name,benchmark,device,config,dataset='cifar100',classes_per_half=None,
                classes_per_task=None,backbone=None,tasks=None):
    from baselines import build_method,build_custom_model,CUSTOM_BACKBONE
    from models import create_model,_INPUT_SIZE
    if classes_per_half is None or classes_per_task is None:
        classes_per_half,classes_per_task=geometry(dataset)
    tasks=tasks or task_count(dataset)
    config={k:v for k,v in config.items() if k not in ('batch_size','scheduler','milestone_count','lr_decay','weight_decay')}
    if name=='spacenet':
        from method_loader import load as _load_m
        _asp=_load_m('baselines/SpaceNet','audited_spacenet');AuditedSpaceNet,SpaceNetMLP=_asp.AuditedSpaceNet,_asp.SpaceNetMLP
        side=_INPUT_SIZE[dataset]
        return AuditedSpaceNet(SpaceNetMLP(3*side*side,classes_per_half,classes_per_task),
                               device,scenario=benchmark.scenario,**config)
    model=(build_custom_model(name,dataset,classes_per_task,benchmark.scenario,tasks) if name in CUSTOM_BACKBONE
           else create_model(dataset,classes_per_task,tasks,benchmark.scenario,backbone))
    if name=='joint':
        from method_loader import load as _load_m
        JointPrefix=_load_m('baselines','joint_audited').JointPrefix
        return JointPrefix(model,device,scenario=benchmark.scenario,**config)
    if name=='uniclun':
        from method_loader import load as _load_m
        UniCLUN=_load_m('baselines/UniCLUN','uniclun').UniCLUN
        return UniCLUN(model,device,scenario=benchmark.scenario,buffer_size=1000,**config)
    options=dict(config);lr=options.pop('lr')
    options.update(num_classes=classes_per_half,buffer_size=1000)
...
def one(args):
    ...
    with ledger.phase('setup',count_flops=False):
        ...
        total_tasks=task_count(dataset)
        classes_per_half,classes_per_task=geometry(dataset)
        if not args.tasks:args.tasks=total_tasks
        benchmark=ContinualLearningBenchmark(dataset,total_tasks,DATA_ROOT,args.seed,
        ...
        method=make_method(args.method,benchmark,device,config,dataset,
            classes_per_half,classes_per_task,getattr(args,'backbone',None),total_tasks)
    ...
    def output_space():
        return (method.full_output_space(total_tasks) if hasattr(method,'full_output_space') else full_space(method.model,total_tasks))
    ...
    for t,(train,val,test) in enumerate(loaders):
        ...
        progress={'method':args.method,'scenario':args.scenario,'half':args.half,'seed':args.seed,
            # Which benchmark this is: a result file has to name its dataset, or
            # runs on different datasets are indistinguishable after the fact.
            'dataset':dataset,'tasks':args.tasks,'classes_per_task':classes_per_task,
            'classes_per_half':classes_per_half,'backbone':getattr(args,'backbone',None),
...
def queue(args):
    ...
    q_half,q_per_task=geometry(args.dataset,args.tasks)
    protocol={'dataset':f'{args.dataset} disjoint halves','classes_per_half':q_half,'tasks':args.tasks,
        'classes_per_task':q_per_task,'D_HT':1,'D_E':2,'split_seed':1234,'seeds':[42,43,44],
    ...
    def run(name,scenario,half,seed,config,tag,gpu_index,flops=False,measured=False):
        ...
        cmd=[sys.executable,'-u',str(Path(__file__)),'--one','--method',name,'--scenario',scenario,
             ...
             '--dataset',getattr(args,'dataset','cifar100')]
        if getattr(args,'backbone',None):cmd+=['--backbone',args.backbone]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(GPUS)*pack) as pool:
        for name in args.methods:
            ...
            for scenario in scenarios:
                ...
                if args.tasks<task_count(args.dataset):
...
def main():
    ...
    p.add_argument('--tasks',type=int,default=0,help='truncate to the first N tasks (pilots); 0 = the protocol count for the dataset');p.add_argument('--flops',action='store_true')
    p.add_argument('--dataset',default='cifar100',choices=sorted(DATASET_CLASSES),
        help='CIFAR-100 (the paper), CIFAR-20 (the 20 superclasses), TinyImageNet-200 or ImageNet-1k')
    p.add_argument('--backbone',default=None,help='override the per-dataset default backbone')
    ...
    args=p.parse_args()
    if not args.tasks:args.tasks=task_count(args.dataset)
