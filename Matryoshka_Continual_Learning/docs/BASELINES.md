...
`audited_gtep.make_method`, which calls `baselines.build_method`.

| Key | Method | Scenarios | Searched hyperparameters (besides lr) | Notes |
|---|---|---|---|---|
| `sgd` | Fine-tuning (lower bound) | CIL, TIL | none | |
| `joint` | Joint training (upper bound) | CIL, TIL | none | `joint_audited.JointPrefix` retrains on all seen tasks after each task. |
...
| `spacenet` | SpaceNet (Sokar et al., 2021) | CIL, TIL | `density_factor`, `rewire_fraction` | Its native MLP (3072-400-400-50) |
| `nispa` | NISPA (Gurbuz & Dovrolis, 2022) | CIL, TIL | `nispa_prune_perc`, `nispa_recovery_perc` | Native conv architecture. It uses its own phase/recovery stopping rule. |
