#!/usr/bin/env bash
# Run every method through the real GTEP worker on a tiny slice of CIFAR-100
...
OUT=${OUT:-runs/smoke}
COMMON="--half 1 --seed 42 --epochs 1 --patience 1 --tasks 2 --smoke_samples 64"
status=0
run() {
...
if python snv_adaptive_run.py --scenario class_il --out "$OUT/snv-class_il" --epochs 2 --patience 1 \
     --tasks 2 --smoke_samples 64 --seed 42 \
