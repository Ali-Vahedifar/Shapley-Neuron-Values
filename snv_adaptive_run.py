...
def make_method(name, benchmark, device, config, dataset='cifar100',
                classes_per_half=None, classes_per_task=None, backbone=None, tasks=None):
    if name != 'snv':
        return _audited_make_method(name, benchmark, device, config, dataset,
                                    classes_per_half, classes_per_task, backbone, tasks)
    from models import create_model
    if classes_per_task is None:
        classes_per_half, classes_per_task = G.geometry(dataset)
    tasks = tasks or G.task_count(dataset)
    ...
    method = SNVAdaptive(
        model=create_model(dataset, classes_per_task, tasks, benchmark.scenario, backbone),
        device=device,
